...
export default function User() {
  ...
  const { mutate } = LOGOUT(IsAuth && IsAuth?.csrf_token);
  ...
  function logout() {
    mutate(); // Make an API request to logout account
  }
            ...
            <Row>
              ...
              <Col className="d-flex align-items-center justify-content-center">
                <nav className="p-2 mt-4">
                  ...
                  >
                    Message <Badge bg="success">{UserData && UserData?.secret_messages.filter(data => data.has_read === 0).length}</Badge>
                  </Button>
                </nav>

                <nav className="p-2 mt-4">
                  <DropdownButton id="dropdown-basic-button" title={`${UserData && UserData?.user_data.name}`} variant="secondary">
                ...
                <div className='p-4'>
                  ...
                  <Form>
                    <Form.Group controlId="exampleForm.ControlInput1" className="d-flex align-items-center">
                      <Form.Control
                        type="text"
                        value={`${location.protocol}//${location.hostname}/secret/${UserData && UserData?.user_data?.message_key}`} // Remove the "${location.port}" in production
                        readOnly
                      />
                      <CopyToClipboard
                        text={`${location.protocol}//${location.hostname}/secret/${UserData && UserData?.user_data?.message_key}`}
                        onCopy={handleCopy} // Remove the "${location.port}" in production
                      >
